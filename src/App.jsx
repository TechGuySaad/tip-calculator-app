...
function App() {
  ...
  function reducer(state,action){
    ...
    switch(action.type){
      ...
      case 'UPDATE_TOTAL_TIP_PER_PERSON':
        return {
          ...state, 
          total_tip_per_person: action.payload
        }

      case 'UPDATE_TOTAL_BILL_PER_PERSON':
        return {
          ...state, 
          total_bill_per_person: action.payload
        }
  ...
  const [values,dispatch] = useReducer(reducer, {bill: 0 , tip: 0, people: 0, total_tip_per_person: 0, total_bill_per_person: 0 })
  ...
  return (
    <div className="app h-screen lg:justify-normal w-full justify-center items-center flex flex-col  bg-customCyan-200  gap-y-12 xs:justify-between xs:pt-12 ">
    <div className="logo">
    <svg className='' xmlns="http://www.w3.org/2000/svg" width="87" height="54">
        <path
          fill="#3D6666"
      ...
      <div className="container flex lg:flex-nowrap w-3/5 h-3/6 bg-white rounded-2xl p-7 gap-x-10 shadow-md sm:w-full xs:w-full xs:h-fit xs:flex-wrap ">
        <Bill dispatch={dispatch}/>

        <CalculateBill values={values} dispatch={dispatch}/>
